...
Route::group(['middleware' => ['maintainance','HtmlSpecialchars']], function () {
    ...
    Route::get('/category/{id}', [HomeController::class, 'category'])->name('api.category');
    ...
    Route::get('/product-by-category/{id}', [HomeController::class, 'productByCategory'])->name('api.product-by-category');

    Route::get('/', [HomeController::class, 'index'])->name('api.home');
    Route::get('/about-us', [HomeController::class, 'aboutUs'])->name('api.about-us');
    Route::get('/contact-us', [HomeController::class, 'contactUs'])->name('api.contact-us');

    Route::get('/blog', [HomeController::class, 'blog'])->name('api.blog');
    Route::get('/blog/{slug}', [HomeController::class, 'blogDetail'])->name('api.blog-detail');
    Route::post('/blog-comment', [HomeController::class, 'blogComment'])->name('api.blog-comment');

    Route::get('/track-order-response/{id}', [HomeController::class, 'trackOrderResponse'])->name('api.track-order-response');
    Route::get('/faq', [HomeController::class, 'faq'])->name('api.faq');
    Route::get('/page', [HomeController::class, 'allCustomPage'])->name('api.custom-page');
    Route::get('/page/{slug}', [HomeController::class, 'customPage'])->name('api.page');
    Route::get('/terms-and-conditions', [HomeController::class, 'termsAndCondition'])->name('api.terms-and-conditions');
    Route::get('/privacy-policy', [HomeController::class, 'privacyPolicy'])->name('api.privacy-policy');
    Route::get('/seller-terms-conditoins', [HomeController::class, 'sellerTemsCondition'])->name('api.seller-terms-conditoins');
    ...
    Route::get('/add-to-cart', [CartController::class, 'addToCart'])->name('api.add-to-cart');
    Route::get('/cart-clear', [CartController::class, 'cartClear'])->name('api.cart-clear');
    Route::get('/cart-item-remove/{id}', [CartController::class, 'cartItemRemove'])->name('api.cart-item-remove');
    Route::get('/cart-item-increment/{id}', [CartController::class, 'cartItemIncrement'])->name('api.cart-item-increment');
    Route::get('/cart-item-decrement/{id}', [CartController::class, 'cartItemDecrement'])->name('api.cart-item-decrement');

    Route::get('/apply-coupon', [CartController::class, 'applyCoupon'])->name('api.apply-coupon');
    Route::get('/calculate-product-price', [CartController::class, 'calculateProductPrice'])->name('api.calculate-product-price');
    Route::get('/test-log', function() {
        \Log::info('Test log entry');
        return response()->json(['message' => 'Test log created']);
    });
    Route::get('/test-simple', function() {
        return response()->json(['message' => 'Simple test works']);
    });

    Route::get('login/google',[LoginController::class, 'redirectToGoogle'])->name('api.login-google');
    Route::get('/callback/google',[LoginController::class,'googleCallBack'])->name('api.callback-google');

    Route::get('login/facebook',[LoginController::class, 'redirectToFacebook'])->name('api.login-facebook');
    Route::get('/callback/facebook',[LoginController::class,'facebookCallBack'])->name('api.callback-facebook');


    Route::get('/login', [LoginController::class, 'loginPage'])->name('api.login');
    Route::post('/store-login', [LoginController::class, 'storeLogin'])->name('api.store-login');
    Route::post('/resend-register-code', [RegisterController::class, 'resendRegisterCode'])->name('api.resend-register-code');
    Route::post('/store-register', [RegisterController::class, 'storeRegister'])->name('api.store-register');
    Route::get('/forget-password', [LoginController::class, 'forgetPage'])->name('api.forget-password');
    Route::post('/send-forget-password', [LoginController::class, 'sendForgetPassword'])->name('api.send-forget-password');
    Route::get('/reset-password/{token}', [LoginController::class, 'resetPasswordPage'])->name('api.reset-password');
    Route::post('/store-reset-password/{token}', [LoginController::class, 'storeResetPasswordPage'])->name('api.store-reset-password');
    Route::get('/user/logout', [LoginController::class, 'userLogout'])->name('api.user.logout');
